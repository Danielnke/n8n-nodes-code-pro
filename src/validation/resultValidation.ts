/**
 * Stock Code–style return shape validation (all-items / each-item).
 */
...
export function validateRunCodeEachItem(
	...
	// Compat: many snippets always `return [ { ... } ]` even for one item.
	...
	if (Array.isArray(executionResult)) {
		...
		if (executionResult.length === 1) {
			...
			throw new CodeProValidationError(
				`Code doesn't return a single object`,
				`An array of ${executionResult.length} items was returned. In "Run Once for Each Item" mode return one object (or a one-element array). Multi-item batch output needs Mode → "Run Once for All Items". Note: Code Pro $input.all() is always the full input list (stock Code semantics).`,
				itemIndex,
			);
		}
	}
	...
	// After normalize, plain objects become { json: {...} } — only validate keys on final shape
